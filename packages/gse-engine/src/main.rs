...
use gse_engine::{EmulatorFlavor, PatchPlan, anticheat, apply_plan, interfaces, restore, scanner};
...
fn main() -> ExitCode {
    ...
    let Some(command) = args.first() else {
        eprintln!("usage: gse-engine <scan|patch|restore|interfaces> [options]");
...
fn cmd_patch(args: &[String]) -> Result<serde_json::Value, String> {
    ...
    let emulator_dir =
        PathBuf::from(flag(args, "--emulator-dir").ok_or("--emulator-dir is required")?);
